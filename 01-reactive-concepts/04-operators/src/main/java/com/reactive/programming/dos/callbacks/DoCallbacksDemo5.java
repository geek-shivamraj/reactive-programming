 ...
 *
 *      - Lifecycle Flow:
 *          - For 4 requests: doFirst --> doOnSubscribe --> doOnRequest --> doOnNext --> doOnComplete --> doOnTerminate --> doFinally
 *          - For next 2 requests (one cancel request): doOnCancel --> doFinally
 *
 */
public class DoCallbacksDemo5 {

    private static final Logger log = LoggerFactory.getLogger(DoCallbacksDemo5.class);
