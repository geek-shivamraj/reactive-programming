 ...
 *
 *      - Lifecycle Flow:
 *          - For first 2 requests: doFirst --> doOnSubscribe --> doOnRequest --> doOnNext --> doOnComplete --> doOnTerminate --> doFinally
 *          - For next 2 requests (one cancel request): doOnCancel --> doFinally
 *          - For last 2 requests (2 discard requests): doOnDiscard
 *
 *      - Interesting behavior
 *          - doOnRequest-2 requests for Long.MAX_VALUE, but doOnRequest-1 requests for 2 becoz of take(2) &
 *              that's why Publisher will just emit 2 items & discard other 2.
 *         - Also since we're using Flux.create(), it doesn't care about the downstream & will emit all the items.
 *         - If we had used Flux.generate(), it would have only emitted requested no. of items.
 *
 */
public class DoCallbacksDemo3 {

    private static final Logger log = LoggerFactory.getLogger(DoCallbacksDemo3.class);
